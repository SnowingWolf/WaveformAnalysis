...
- `risk_level`: `medium`
- `scope_in`: Offline `plugins-web` lineage SVG, documentation completeness and DAG impact scores, static navigation, source tests, and CLI documentation. The global graph must omit isolated plugins from the SVG, list them separately, omit runtime-resolved placeholders, provide zoom/reset controls, and accept a plugin-focused URL from a local-lineage page.
...
- `source_change_summary`: Add a generated clickable plugin DAG and two documentation-only scores. Keep the graph readable by rendering only declared connected dependencies; surface isolated plugins in a compact list and link each plugin-local DAG to a focused global graph view.
