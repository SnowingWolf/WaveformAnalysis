...
> ⚠️ `WaveformPreviewer`、`preview_waveforms()` 和 `plot_records_waveforms()` 自 `0.2.0` 起已弃用。
> 当前页面仅为兼容保留，不再作为主导航入口，下个 major 版本计划删除这些 API。
> 该功能没有直接替代接口；如需继续使用类似能力，请改用底层数据访问接口并自行编写 Matplotlib 绘图逻辑。
