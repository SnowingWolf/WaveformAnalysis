# 工具函数

**导航**: [文档中心](../../README.md) > [功能特性](../README.md) > 工具函数

DAQ 适配器、DAQ 运行分析与事件筛选等实用工具。

## 文档列表

| 文档 | 说明 |
|------|------|
| [DAQ 适配器](../../plugins/reference/ADAPTER_SYSTEM_GUIDE.md) | DAQ 适配器层使用指南（格式读取、目录结构） |
| [DAQ 运行分析器](DAQ_ANALYZER_GUIDE.md) | DAQ 运行概览与统计导出 |
| [Matplotlib PDF 导出](PDF_EXPORT_GUIDE.md) | 将单个或多个 Figure 导出为 PDF |

## 学习路径

1. [DAQ 适配器](../../plugins/reference/ADAPTER_SYSTEM_GUIDE.md) - DAQ 数据格式适配
2. [DAQ 运行分析器](DAQ_ANALYZER_GUIDE.md) - 运行概览与统计
3. [Matplotlib PDF 导出](PDF_EXPORT_GUIDE.md) - 生成单页或多页分析图报告

## 相关资源

- [高级功能](../advanced/README.md) - 执行器管理、缓存机制
- [插件功能](../../plugins/README.md) - 信号处理、流式插件
