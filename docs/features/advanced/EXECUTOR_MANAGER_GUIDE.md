...
全局执行器管理框架提供统一的接口来管理线程池和进程池，支持资源重用、自动清理和配置管理。它采用单例模式，确保全局唯一的管理器实例，便于在整个应用中统一管理并行执行资源。
...
v1.2.0 release gate 覆盖了执行器管理和存储后端相关变更。并行执行入口仍应通过统一
管理器获取和释放执行器，避免在缓存或数据访问路径中散落创建线程池/进程池。
...
- `get_config(name)` - 获取预定义配置
- `register_config(name, config)` - 注册自定义配置
