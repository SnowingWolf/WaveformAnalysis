**导航**: [文档中心](../../README.md) > [功能特性](../README.md) > [高级功能](README.md) > 缓存系统
...
- **并发安全**: 存储使用基于文件的锁文件（`.lock`）来协调写入，并带有过期检测和 PID 存活检查以减少死锁的风险；但这仍然不是跨节点或网络文件系统下的可靠互斥机制。建议避免多个进程同时写入同一键，或者在多主机场景下采用外部协调（例如 Redis 锁或数据库行锁）。
