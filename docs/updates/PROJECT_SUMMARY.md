**导航**: [文档中心](../README.md) > [更新记录](README.md) > 项目重组总结
...
### 8. 架构加固与鲁棒性提升 (2025-12)

针对大规模数据处理和复杂插件依赖，进行了深度的架构加固：

- ✅ **无状态 Context**: 引入显式 `run_id` 传递，彻底隔离不同运行的数据状态。
- ✅ **重入保护**: 自动检测插件依赖链中的循环调用，防止死循环。
- ✅ **侧效应隔离**: 为副作用插件提供独立的输出目录，确保数据纯净。
- ✅ **鲁棒哈希系统**: 
    - 引入插件版本号 (`version`)。
    - 标准化 DType 表达 (`dtype.descr`)，消除跨平台/版本差异。
    - 增加血缘校验机制，逻辑变更自动失效缓存。
- ✅ **执行安全性**: 
    - 增加 `on_error` 和 `cleanup` 钩子。
    - 实现输出契约校验（DType 静态与动态验证）。
    - 原子性文件写入（`.tmp` 机制）。
- ✅ **数据摘要自动化**: `WaveformDataset.get_data()` 会自动调用 `_ensure_*` 步骤，确保 `df`/`df_events`/`df_paired` 在任意上下文中都可以直接获取，无需手动先后执行各个步骤。
- ✅ **流式生成器耐久性**: `Context` 不再把生成器结果绑定成类属性，且再次获取 `stream` 输出时会重新构建生成器；流式插件（`hits_stream`、`st_waveforms_stream`）现在每次都能重跑而不被单次消费的迭代器“卡死”，极大增强多次分析场景。
