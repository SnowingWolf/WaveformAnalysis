...
export function PluginDirectory({ plugins }: { plugins: PluginModel[] }) {
  ...
  return <div className="plugin-directory">
    ...
    <div className="plugin-table"><div className="plugin-table__head"><span>插件</span><span>描述</span><span>输出</span><span>版本</span></div>{visible.map((plugin) => <Link className="plugin-table__row" key={plugin.provides} href={plugin.route}><span className="plugin-name"><Icon name="puzzle" size={16} /><strong>{plugin.provides}</strong></span><span>{plugin.summary}</span><span className="mono">{plugin.outputKind}</span><span className="mono">{plugin.provides === "records" ? `v${plugin.version}` : "—"}</span></Link>)}{visible.length === 0 && <p className="empty-state">没有匹配的插件。</p>}</div>
  </div>;
}
