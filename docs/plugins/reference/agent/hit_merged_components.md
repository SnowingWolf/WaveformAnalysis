...
| Depends On | `hit_threshold`, `hit_merged` |
...
说明：
按 `component_offset/component_count` 的顺序平铺每个 `hit_merged` 的组件 hit 索引。消费方可用 `hit_index` 回查 `hit_threshold`，再用 `record_id + record_sample_* / wave_pool_*` 取真实波形窗口。
