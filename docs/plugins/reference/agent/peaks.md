...
declared_depends_on: ["peaklets", "peaklet_features"]
resolved_depends_on: ["peaklets", "peaklet_features"]
...
source_fingerprint: "18ef6786103195d9899435cebd0751b85f17d3aca1575a79ee4ce85f810173fe"
generated: true
---
# peaks
...
当 `peaklet_features` 与 `peaklets` 行数相同且 `peak_id` 严格等于行索引时，PeaksPlugin 直接复用原特征行；其余输入使用稳定排序 + `searchsorted` 按 `peak_id` 对齐。任何 peaklet 缺失对应特征都会被认定为数据不一致并抛出异常，从而保证 peaks 表总是完整、且与 peaklets 一一对齐。
...
| Source Fingerprint | `18ef6786103195d9899435cebd0751b85f17d3aca1575a79ee4ce85f810173fe` |
...
2. 检测特征行：若特征行数与 peaklet 行数相同且 `peak_id` 严格等于行索引，直接使用原特征行；
3. 回退对齐：否则对 `peaklet_features['peak_id']` 作稳定排序（mergesort），再用 `searchsorted` 定位每个 peaklet 的特征行；缺失特征时抛错。
...
直接消费者：`peak_classification`、`s1_s2_pair_candidates`- `peak_classification` 直接以 peaks 特征做 S1/S2 分类，任何特征字段语义变化都会改变分类结果。
