...
|------|-------|
| Provides | `peaklets` |
| Depends On | `hit_merged`, `hit_merged_components`, `hit_merged_features`, `records`, `wave_pool` |
...
`peaklets` 依赖链入口：
`hit_merged -> hit_merged_components -> hit_merged_features -> records -> wave_pool -> peaklets`
