...
class Context(CacheMixin, PluginMixin):
    ...
    def _get_data_from_memory(self, run_id: str, name: str) -> Any:
        ...
        if name not in reserved and not hasattr(self.__class__, name) and hasattr(self, name):
    ...
    def get_lineage(self, data_name: str) -> Dict[str, Any]:
        """
        ...
        lineage = {
            ...
            "depends_on": {dep: self.get_lineage(dep) for dep in plugin.depends_on},
        }
    ...
    def plot_lineage(self, data_name: str, kind: str = "labview", **kwargs):
        ...
        """
        from .models import build_lineage_graph
