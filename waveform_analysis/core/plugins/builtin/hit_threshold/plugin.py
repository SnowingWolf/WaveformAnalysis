...
本模块包含：
1. THRESHOLD_HIT_DTYPE 契约 dtype
2. ThresholdHitPlugin: 纯阈值 hit 插件（provides='hit_threshold'），输出 THRESHOLD_HIT_DTYPE

旧导入路径兼容：
- ``HitFinderPlugin``（旧 alias）保留在 hit_finder shim 中再导出，
  推荐改为 hit bundle 的 canonical HitFinderPlugin。
- Numba 共享内核位于本 bundle 的 ``_compute``（原 hit_threshold_numba.py）。
...
class ThresholdHitPlugin(BatchProcessingPlugin):
    ...
    **重要 - 输出顺序说明:**
    - 输出的 hits 按 record 输入顺序连接，**不保证**按全局时间戳排序
    - 单个 record 内的 hits 按 sample position 有序（时间递增）
    - 跨 records 时，如果输入 records 的时间戳乱序，hits 也会乱序
    - 下游组件（如 PeakChannelAccessor）在拼接跨 records 波形时会按时间排序
    - 如果需要时间有序的 hits，应在使用前按 'timestamp' 字段排序
    """
    ...
    def _compute_records_batched(
        ...
        # 连接所有 batch 的 hits
        # 注意：结果按 batch/record 顺序连接，不按时间戳排序
        # 下游组件需要自行处理排序（如 PeakChannelAccessor._extract_multi_record_waveform）
        if not all_hits:
            return _empty_hits()
    ...
    def _compute_streaming(
        ...
        # streaming 模式：连接所有 chunk 的 hits
        # 注意：结果按 chunk 顺序连接，不按时间戳排序
        if all_hits:
