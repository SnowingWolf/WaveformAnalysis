...
_IDENTITY_CHECK_CHUNK_SIZE = 1 << 20


def _has_identity_peaklet_ids(feature_peaklet_ids: np.ndarray, n_peaklets: int) -> bool:
    """Return whether feature rows are exactly aligned to peaklet row indices.

    Check bounded chunks so validating a production-sized feature table does not
    require allocating another full-sized index array.
    """
    if feature_peaklet_ids.dtype != np.dtype(np.int64) or len(feature_peaklet_ids) != n_peaklets:
        return False

    for start in range(0, n_peaklets, _IDENTITY_CHECK_CHUNK_SIZE):
        stop = min(start + _IDENTITY_CHECK_CHUNK_SIZE, n_peaklets)
        expected_ids = np.arange(start, stop, dtype=np.int64)
        if not np.array_equal(feature_peaklet_ids[start:stop], expected_ids):
            return False
...
class PeaksPlugin(Plugin):
    ...
    provides = "peaks"
    depends_on = ["peaklets", "peaklet_features"]
    ...
    agent_doc = {
        "overview": (
            ...
            "当 `peaklet_features` 与 `peaklets` 行数相同且 `peak_id` 严格等于行索引时，"
            "PeaksPlugin 直接复用原特征行；其余输入使用稳定排序 + `searchsorted` 按 `peak_id` "
            "对齐。任何 peaklet 缺失对应特征都会被认定为数据不一致并抛出异常，从而保证 peaks "
            "表总是完整、且与 peaklets 一一对齐。\n\n"
        ...
        "workflow_steps": [
            ...
            "检测特征行：若特征行数与 peaklet 行数相同且 `peak_id` 严格等于行索引，直接使用原特征行；",
            "回退对齐：否则对 `peaklet_features['peak_id']` 作稳定排序（mergesort），再用 `searchsorted` 定位每个 peaklet 的特征行；缺失特征时抛错。",
    ...
    def compute(self, context: Any, run_id: str, **_kwargs) -> np.ndarray:
        ...
        feature_peaklet_ids = features["peak_id"]
        if _has_identity_peaklet_ids(feature_peaklet_ids, len(peaklets)):
            aligned_features = features
            peaklet_ids = feature_peaklet_ids
        else:
            feature_peaklet_ids = feature_peaklet_ids.astype(np.int64, copy=False)
        ...
        out = np.empty(len(peaklets), dtype=PEAKS_DTYPE)
