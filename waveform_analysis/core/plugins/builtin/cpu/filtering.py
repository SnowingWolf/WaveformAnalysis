...
class FilteredWaveformsPlugin(Plugin):
    ...
    agent_doc = {
        "overview": (
            "FilteredWaveformsPlugin 对 `st_waveforms` 中每个事件的波形应用数字滤波，输出与输入"
            "同构、仅将 `wave` 字段替换为 float32 的结构化数组。它支持两种滤波引擎：Butterworth "
            "带通滤波（'BW'，先经 `scipy.signal.butter` 设计 SOS 再 `sosfiltfilt` 零相位滤波）与 "
            "Savitzky-Golay 平滑（'SG'，`savgol_filter`），并按 (board, channel) 分组、可选分批"
            "与并行地处理。\n\n"
            "该插件与 records-backed 的 `wave_pool_filtered`（WavePoolFilteredPlugin）共享 "
            "filtering 模块的配置解析与通道覆盖逻辑；`channel_config` 可按 (board, channel) 覆盖"
            "滤波参数，`max_workers`/`batch_size` 控制并行与分批策略。\n\n"
            "输出保留 st_waveforms 的所有非 wave 字段（baseline、timestamp、record_id、board、"
            "channel 等），保持行语义不变；`save_when='target'` 意味着它只在被目标消费时物化，"
            "例如 `signal_peaks_stream` 流式峰值检测与 `waveform_width`（use_filtered=True 时）"
            "都以 `filtered_waveforms` 作为滤波波形源。"
        ),
        "workflow_steps": [
            "读取输入：从 context 获取 `st_waveforms`，校验其为结构化数组且包含 `channel` 与 `wave` 字段。",
            "确定输出 dtype：按输入 dtype 构造 `wave` 为 float32 的滤波 dtype（`create_filtered_waveform_dtype`）；空输入直接返回空数组。",
            "复制非波字段：将 `st_waveforms` 除 `wave` 外的所有字段原样复制到输出，保持行对齐。",
            "解析滤波配置：按 (board, channel) 分组（`build_filter_batches`，可含 `channel_config` 通道覆盖），对每个批次解析并校验 BW/SG 参数。",
            "执行滤波：`max_workers` 允许且批次多于 1 时用线程池 `parallel_map` 并行，否则串行；对每个批次的 `wave` 应用 `_filter_channel`（sosfiltfilt / savgol_filter）。",
            "写回输出：将滤波后的 float32 波形写回 `output['wave']` 的对应批次切片，返回结果。",
        ],
        "behavior_notes": [
        ...
        "field_notes": {
            "wave": "float32 滤波后的波形样本数组（形状与 st_waveforms 的 wave 相同），是唯一被滤波修改的字段。",
            "baseline": "float64 基线，从 `st_waveforms` 原样复制。",
            "baseline_upstream": "float64 上游基线，从 `st_waveforms` 原样复制（若存在）。",
            "polarity": "硬件真实极性，从 `st_waveforms` 原样复制。",
            "timestamp": "ps 级 ADC 时间戳，从 `st_waveforms` 原样复制。",
            "record_id": "顺序记录 id，从 `st_waveforms` 原样复制。",
            "dt": "采样间隔（ns），从 `st_waveforms` 原样复制。",
            "event_length": "事件长度（采样点数），从 `st_waveforms` 原样复制。",
            "board": "板卡索引，从 `st_waveforms` 原样复制。",
            "channel": "物理通道号，从 `st_waveforms` 原样复制。",
        },
        "config_notes": {
            "filter_type": "滤波器类型：'BW'（Butterworth 带通）或 'SG'（Savitzky-Golay）。",
            "lowcut": "BW 低频截止频率。",
            "highcut": "BW 高频截止频率，必须小于奈奎斯特频率 fs/2。",
            "fs": "BW 采样率（GHz）。",
            "filter_order": "BW 滤波器阶数。",
            "sg_window_size": "SG 窗口大小（奇数；偶数会自动上取整）。",
            "sg_poly_order": "SG 多项式阶数，必须小于窗口大小。",
            "max_workers": "并行工作线程数；None 使用 CPU 核心数，1 或 0 禁用并行。",
            "batch_size": "每批次事件数（0 表示不分批，整个通道一次处理）。",
            "channel_config": "按 (board, channel) 的插件通道覆盖配置，可覆盖滤波参数。",
        },
        "failure_modes": [
        ...
        "agent_change_notes": [
            "滤波参数语义变更会影响 BW/SG 输出特性并级联到 `signal_peaks_stream`、`waveform_width` 与共享的 records 侧 `wave_pool_filtered`，请同步运行对应定向测试并重新生成文档。",
        ],
    }
