...
class WaveformLoader:
    ...
    def get_raw_files(self, daq_run: Optional[Any] = None, daq_info: Optional[Dict] = None) -> List[List[str]]:
        ...
        return [[fp for _, fp in sorted(groups.get(ch, []))] for ch in range(max_ch + 1)]
...
@export
def get_files_by_filetime(indexed_table: List[List[Tuple[float, str]]], t_query_mtime: float) -> Dict[int, str]:
    """
...
@export
def get_files_before(raw_filess: List[List[str]], files_by_time: Dict[int, str]) -> List[List[str]]:
    """获取指定文件之前的所有文件。"""
    sel_raw_filess = []
