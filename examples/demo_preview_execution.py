...
def example_different_verbosity():
    ...
    ctx.set_config(
        {"filter_type": "BW", "lowcut": 0.05, "highcut": 0.8}, plugin_name="filtered_waveforms"
    )
...
def example_programmatic_use():
    ...
    # 检查需要计算的插件数量
    needs_compute = [
        plugin for plugin, status in result["cache_status"].items() if status["needs_compute"]
    ]
...
def example_compare_multiple_targets():
    ...
    for target in targets:
        result = ctx.preview_execution(
            "run_001", target, show_tree=False, show_config=False, show_cache=False, verbose=0
        )
