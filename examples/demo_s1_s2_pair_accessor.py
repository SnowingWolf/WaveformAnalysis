...
替代用户原有的手动函数：
- get_peaklet_waveform_by_peak_id() → accessor.waveform()
- plot_s1_s2_pair_on_timeline() → accessor.plot()
"""
...
def example_basic_usage(context, run_id):
    ...
    if len(accessor.pairs) > 0:
        pair = accessor.pair(int(accessor.pairs[0]["pair_id"]))
        print(f"\n配对 {pair['pair_id']} 信息:")
    ...
    if len(accessor.pairs) > 0:
        ...
        pairs_for_s1 = accessor.pairs_for_s1(s1_peak_id)
...
def example_filtering(context, run_id):
    ...
    print(f"mask: 找到 {len(filtered)} 个符合条件的配对")
    ...
    filtered3 = accessor.pairs[accessor.mask(custom_filter=custom_filter)]
...
def example_waveform_extraction(context, run_id):
    ...
    if len(accessor.pairs) > 0:
        ...
        # 提取单个 peak 的波形
        s1_wf = accessor.waveform(int(pair["s1_peak_id"]))
        if s1_wf:
            print("S1 波形:")
        ...
        # 注意：默认返回 view，不应原地修改
        # 如果需要修改，使用 copy=True
        _s1_wf_copy = accessor.waveform(int(pair["s1_peak_id"]), copy=True)  # noqa: F841
...
def example_visualization(context, run_id):
    ...
    if len(accessor.pairs) > 0:
        ...
        fig, ax = accessor.plot(pair_id, pad_ns=200)
        plt.savefig(f"output/pair_{pair_id}.png", dpi=150, bbox_inches="tight")
        plt.close(fig)
        print(f"已保存配对 {pair_id} 的图像")

        # 批量绘制（简单循环）
        filtered = accessor.pairs[accessor.mask(drift_time_ns_range=(10000, 50000))]
        for _i, pair in enumerate(filtered[:5]):
            fig, _ax = accessor.plot(pair, pad_ns=200)
...
def main():
    ...
    print("\n# 过滤配对")
    print("filtered = accessor.pairs[accessor.mask(drift_time_ns_range=(10000, 50000))]")
    print("\n# 绘制配对")
    print("fig, ax = accessor.plot(pair_id=42)")
    print("plt.show()")
