...
def test_parse_and_stack_files_with_mixed_headers(tmp_path):
    ...
    assert (
        result.shape[1] >= 13
    ), f"Expected at least 13 columns (3 metadata + 10 samples), got {result.shape[1]}"
    ...
    ), f"Timestamps mismatch: {timestamps} vs {expected_timestamps}"
...
def test_parse_and_stack_files_parallel_processing(tmp_path):
    ...
    assert np.array_equal(
        result_parallel, result_serial
    ), "Parallel and serial results should match"
...
def test_parse_and_stack_files_channel_independence(tmp_path):
    ...
    assert not np.array_equal(
        timestamps_ch0, timestamps_ch1
    ), "Different channels should have different timestamps"
