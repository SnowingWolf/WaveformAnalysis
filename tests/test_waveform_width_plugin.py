...
def test_waveform_width_plugin_basic(mock_context, synthetic_waveform):
    ...
    ) < width_data["total_width"] * 0.2  # 允许 20% 误差


def test_waveform_width_plugin_empty_channel():
    """测试空通道的处理"""
    ctx = Context()
    ctx.register_plugin(WaveformWidthPlugin())
...
def test_waveform_width_plugin_sampling_rate():
    """测试采样率配置对时间计算的影响"""
    ctx = Context()
    ctx.register_plugin(WaveformWidthPlugin())
...
def test_waveform_width_plugin_dependencies():
    """测试插件依赖关系"""
    plugin = WaveformWidthPlugin()

    assert plugin.provides == "waveform_width"
    assert "signal_peaks" in plugin.depends_on
...
def test_waveform_width_plugin_options():
    ...
    assert plugin.options["sampling_rate"].default == 1.0
