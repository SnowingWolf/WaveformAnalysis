...
class TestIntegrityChecker:
    ...
    def test_scan_directory(self):
        ...
        for _filename, info in results.items():
...
class TestStorageWithChecksum:
    ...
    def test_storage_with_checksum(self):
        """测试启用checksum的存储"""
        storage = MemmapStorage(self.temp_dir, enable_checksum=True, checksum_algorithm="sha256")
    ...
    def test_storage_verify_on_load(self):
        """测试加载时验证checksum"""
        # Save with checksum
        storage_save = MemmapStorage(self.temp_dir, enable_checksum=True, verify_on_load=False)

        data = np.array([1, 2, 3, 4, 5], dtype=np.int32)
        storage_save.save_memmap("test_key", data, run_id=self.test_run_id)

        # Load with verification enabled
        storage_load = MemmapStorage(self.temp_dir, enable_checksum=True, verify_on_load=True)
    ...
    def test_storage_detect_corruption(self):
        """测试检测数据损坏"""
        # Save with checksum
        storage = MemmapStorage(self.temp_dir, enable_checksum=True, verify_on_load=True)
    ...
    def test_storage_checksum_with_compression(self):
        ...
        storage = MemmapStorage(
            self.temp_dir, compression="gzip", enable_checksum=True, checksum_algorithm="sha256"
        )
    ...
    def test_verify_integrity_method(self):
        ...
        storage = MemmapStorage(self.temp_dir, enable_checksum=True)
