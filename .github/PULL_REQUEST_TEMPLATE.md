...
<!--
此表是合并前必填项。必须覆盖 GitHub 在此 PR 中列出的全部 commit；每项使用完整 40 位 SHA，且每个 SHA 只能出现一次。
“变更目的和影响”说明该 commit 为什么存在、改变了什么；“验证证据”填写实际命令和结果，或说明未运行的理由。不要保留占位文字。
提交集合会在每次 push、rebase、目标分支更新或编辑 PR 描述后由 CI 重新校验。
-->

| Commit SHA | 变更目的和影响 | 验证证据 |
| --- | --- | --- |
| `<完整 40 位 SHA>` | `<该 commit 的目的、主要改动和行为影响>` | `<执行的检查及结果，或未执行原因>` |
